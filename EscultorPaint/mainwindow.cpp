...
void MainWindow::on_createOutputFile_released()
{
    if(this->sculptor != nullptr)
    {
        ...
        if(objFlDlg.exec()){
            ...
            QString fileNameVec = objFlDlg.selectedFiles().at(0)+".vec";
            string filenameOFF_std(fileNameOff.toStdString());
            string filenameVEC_std(fileNameVec.toStdString());
            this->sculptor->writeOFF(filenameOFF_std);
            this->sculptor->writeVECT(filenameVEC_std);
        }
    }
    else
    {
